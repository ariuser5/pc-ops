...
- `report [--from yyyy-MM-dd|.|today] [--to yyyy-MM-dd|.|today] [--detailed]`: show a time summary for a date interval. `.` and `today` are aliases for the current local date. When `--from` is provided without `--to`, the interval ends today. With `--detailed`, the output includes a per-day breakdown followed by interval totals per task. If no dates are provided, it reports today.
