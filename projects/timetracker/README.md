...
dotnet run --project ./projects/timetracker/src/TimeTracker.csproj -- task set Ticket-123 --date yesterday --from 09:00 --to 11:00
...
dotnet run --project ./projects/timetracker/src/TimeTracker.csproj -- break set --date -2d --from 12:00 --to 12:30
dotnet run --project ./projects/timetracker/src/TimeTracker.csproj -- break remove --from 12:00 --to 12:30
dotnet run --project ./projects/timetracker/src/TimeTracker.csproj -- break remove --date 2026-07-08 --from 12:00 --to 12:30
...
- `task set [task-name] [--date <date>] [--from HH:mm|HH;mm|yyyy-MM-ddTHH:mm] [--to HH:mm|HH;mm|yyyy-MM-ddTHH:mm]`: the canonical task command.
	Without `--to`, it sets the current task and optionally backdates its start.
	With `--from` and `--to`, it assigns a bounded task interval. Use `--date` with time-only values to correct a past day; historical corrections require `--to`.
- `break set [--date <date>] --from HH:mm|HH;mm|yyyy-MM-ddTHH:mm --to HH:mm|HH;mm|yyyy-MM-ddTHH:mm [--daily] [--name <id>]`: set a one-off break, or a recurring daily break rule when `--daily` is present. Use `--date` with time-only values for a past one-off break. `--date` cannot be combined with `--daily`. Use `--name` to provide a custom unique id for a recurring rule.
- `break remove [--date <date>] --from HH:mm|HH;mm|yyyy-MM-ddTHH:mm --to HH:mm|HH;mm|yyyy-MM-ddTHH:mm`: remove a one-off break interval on today or the selected date.
...
- `task set --from ... --to ...` assigns a bounded slice of time to a task. If the same task was already active earlier, moving `--from` later naturally restores the earlier slice to the previous task. Add `--date yesterday` or another date selector to use time-only bounds on a past day.
